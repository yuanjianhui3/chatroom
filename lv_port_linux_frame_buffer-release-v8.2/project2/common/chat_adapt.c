...
void Dir_Look_Append_ChatBtn(struct Ui_Ctrl *UC_P, lv_obj_t *scr_home) 
{
    ...
    if(album_btn == NULL) {
        printf("未找到相册按钮，使用默认位置创建聊天室按钮\n");
        // 使用默认位置创建聊天室按钮
        lv_obj_t *chat_btn = lv_btn_create(scr_home);
        lv_obj_set_size(chat_btn, 80, 80);
        lv_obj_set_style_bg_opa(chat_btn, 0, 0);
        lv_obj_set_pos(chat_btn, 100, 300); // 默认位置
        
        // 创建聊天室按钮图片
        lv_obj_t *chat_img = lv_img_create(chat_btn);
        lv_img_set_src(chat_img, "S:/8080chat.png");
        lv_obj_set_style_img_opa(chat_img, 180, 0);
        lv_obj_set_size(chat_img, 80, 80);
        lv_obj_center(chat_img);
        
        // 创建聊天室按钮标签
        lv_obj_t *chat_lab = lv_label_create(scr_home);
        lv_obj_set_pos(chat_lab, 100, 390);
    ...
    // 4. 计算位置：相册按钮右侧，间距50px
    lv_obj_set_pos(chat_btn, album_x + album_w + 50, album_y);
    
    // 5. 创建聊天室按钮图片（复用相册图片设置方式）
    lv_obj_t *chat_img = lv_img_create(chat_btn);

    // 检查图片是否存在，如果不存在使用默认图标
    if(access("S:/8080chat.png", F_OK) != 0) {
        printf("聊天室图标不存在，使用默认图标\n");
        lv_img_set_src(chat_img, "S:/8080icon_img.jpg"); // 使用相册图标作为备用
    } else {
    ...
    lv_coord_t chat_lab_x = album_x + album_w + 50;
    lv_coord_t chat_lab_y = album_y + album_h + 30;
